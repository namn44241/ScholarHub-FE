...
const CommentSection = ({ postId }: ICommentSectionProps) => {
  ...
  const isReplyComment = (content: string) => {
    return content.startsWith('@');
  };

  if (isLoading) {
    return (
      <div className="w-full space-y-4 mt-2 pt-3 border-muted-foreground/10 border-t">
    ...
    <div className="w-full space-y-4 mt-2 pt-3 border-muted-foreground/10 border-t">
      ...
      <div className="space-y-3 w-full">
        {comments.map((comment: IComment) => {
          const isReply = isReplyComment(comment.content);
          
          return (
          <div 
            key={comment.id} 
            className={`w-full ${isReply ? 'pl-10' : ''}`}
          >
            <div className="flex gap-2 w-full">
              <Avatar className="w-8 h-8 flex-shrink-0">
                <AvatarImage
              ...
              <div className="flex-1 w-full">
                <div className="bg-muted/50 p-2 rounded-lg w-full">
                ...
                {/* Reply input */}
                {replyingTo === comment.id && (
                  <div className="flex gap-2 mt-2 ml-2 w-full">
