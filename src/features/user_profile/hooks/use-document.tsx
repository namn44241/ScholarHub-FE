...
export const usePostDocument = () => {
  ...
  return useMutation({
    mutationFn: async (data: { type: string; file_path: string; file_name?: string }) => {
...
export const usePutDocument = () => {
  ...
  return useMutation({
    mutationFn: async (data: { id: string; type?: string; file_path?: string; file_name?: string }) => {
  ...
  const submitForm = async (values: DocumentFormValues) => {
    try {
      ...
      if (documentFiles && documentFiles.length > 0) {
        const uploadResult = await uploadDocumentMutation.mutateAsync(documentFiles[0])
